$(document).ready( function() {					//OKIENKO DO DATY
		...
		$('#dateajax').on('change', function () {		// AJAX na zmiane inputa godzinowego po wybieraniu dnia
        	...
        	$.ajax({
	            ...
	            success: function( data ) {
	            	...
	            	var i=0;
	            	$('.hours').append($("<label class='form-control-label' for='hours'>Wybierz Godzinę: </label> <select class='custom-select form-control' id='free_hours' name='hours'><option selected>Godzina rezerwacji</option>").fadeIn('400'))
	            	while(single_hour[i]!=null){			// TWORZENIE GODZIN z tablicy
		                $('#free_hours').append($('<option>', {
						    value: single_hour[i],  				//tylko wartość bez tekstu
						    text: single_hour[i] + single_text[i], // single_text tylko gdy godziny są wolne
						}));
						i++;
					};
            	}
		...
		$('.hours').on('change','#free_hours', function(){   // Tworzenie inputa po wybraniu godziny !
			...
			if((($text.indexOf('Brak'))<=-1) && ($text!="Godzina rezerwacji")){
				...
				$('#hours').append($("<div id='slider-hours'></div><label for='hour_amount'>Ile godzin? : &nbsp</label><input type='text' class=' form-group' id='hour_amount' name='slider-hours' readonly style='margin-top: 25px; border:0; color:#f6931f;'>").fadeIn('400'));
