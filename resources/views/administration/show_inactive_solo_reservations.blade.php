...
<div class="col-md-11 center " style="margin-top: 3rem">
	@if($role=='klient')
		<h3>Nieaktywne rezerwacje użytkownika na obiekcie {{str_replace('_', ' ', $name)}}</h3>
	@else
					...
					<tbody>
						<tr>
							...
							<td>		
								...
								@endif	
								<button type="button" class="btn btn-light btn-sm" title="Usuń"  onclick="location.href='{{url('/users_list/inactive_reservations/solo/remove/'.$name.'/'.$reservation->user_id.'/'.$reservation->reservation_id)}}'" ><i class="fa fa-trash" aria-hidden="true"></i></button> 
							</td>
						</tr>
					</tbody>
					...
					@foreach($active_reservations as $active_reservation)
						...
						<tbody class="{{$active_reservation->reservation_id}}" style=" font-size:10px; display: none;">
							<tr>
								...
								@endif
								<td><button type="button" class="btn btn-light btn-sm" title="Usuń"  onclick="location.href='{{url('/users_list/inactive_reservations/solo/'.$name.'/remove/'.$active_reservation->user_id.'/'.$active_reservation->reservation_id.'/'.$active_reservation->reservation_start)}}'" ><i class="fa fa-trash" aria-hidden="true"></i></button></td>
							</tr>
						</tbody>
						@endif
					@endforeach
