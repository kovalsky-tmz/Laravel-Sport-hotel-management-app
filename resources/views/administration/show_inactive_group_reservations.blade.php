...
<div class="col-md-11 center " style="margin-top: 3rem">
	@if($role=='klient')
		<h3>Niektywne rezerwacje użytkownika na obiekcie {{str_replace('_', ' ', $name)}}</h3>
	@else
					...
					@foreach($active_reservations as $active_reservation)
						...
						<tbody class="{{$active_reservation->reservation_id}}" style=" font-size:10px; display: none;">
							<tr>
								...
								@endif
								<td><button type="button" class="btn btn-light btn-sm" title="Usuń"  onclick="location.href='{{url('/users_list/inactive_reservations/group/'.$name.'/remove/'.$active_reservation->user_id.'/'.$active_reservation->reservation_id.'/'.$active_reservation->reservation_start)}}'" ><i class="fa fa-trash" aria-hidden="true"></i></button></td>
							</tr>
						</tbody>
						@endif
					@endforeach
